...
<script>
    ...
    var  m = [20, 30, 30, 60], w, h, x, y, z, color, xAxis, xAxisScaler, yAxis, stack, nest, line, lineAvg, area, svg_board, svg_ghost, svg_karma, svg_total, layers;
    ...
    function render() {
        ...
        svg_ghost = d3.select("#graphs").append("svg")
            .attr("width", w + m[3] + m[1])
            .attr("height", h + m[0] + m[2])
            .append("g")
            .attr("transform", "translate(" + m[3] + "," + m[0] + ")");

        layers = stack(nest.entries(data_ghost));
        x.domain(d3.extent(data_ghost, function (d) {
            return d.time;
        }));
        y.domain([0, d3.max(data_ghost, function (d) {
            return d.y + d.y0 + 2;
        })]).range([h, 0]);
        ...
        svg_karma = d3.select("#graphs").append("svg")
            .attr("width", w + m[3] + m[1])
            .attr("height", h + m[0] + m[2])
            .append("g")
            .attr("transform", "translate(" + m[3] + "," + m[0] + ")");

        layers = stack(nest.entries(data_karma));
        x.domain(d3.extent(data_karma, function (d) {
            return d.time;
        }));
        y.domain([0, d3.max(data_karma, function (d) {
            return d.y + d.y0 + 2;
        })]);
        ...
        svg_karma.append("g")
            .attr("class", "x axis")
            .attr("transform", "translate(0," + h + ")")
            .call(d3.svg.axis().scale(x).orient("bottom").ticks(d3.time.months, ticks));
